# -*- coding: utf-8 -*-
"""
Created on Sat Nov  3 19:20:27 2018
...
sentences = ["%s %s %s" % (sentence_start_token, x, sentence_end_token) for x in tokenized_data]
...
loss_for_plot = []
def train_with_sgd(model, X_train, y_train, learning_rate=0.005, nepoch=100, evaluate_loss_after=2):
    # We keep track of the losses so we can plot them later
    losses = []
    num_examples_seen = 0
    for epoch in range(nepoch):
        # Optionally evaluate the loss
        print(epoch, "th iteration")
...
np.random.seed(10)
model = RNNVanilla(vocabulary_size)
print(model.sgd_step(XTrain[10], yTrain[10], learning_rate=0.005))

np.random.seed(10)
# Train on a small subset of the data to see what happens
model = RNNVanilla(vocabulary_size)
losses = train_with_sgd(model, XTrain, yTrain, nepoch=6, evaluate_loss_after=2)
